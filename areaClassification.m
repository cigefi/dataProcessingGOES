function [] = areaClassification(dirName)
    if nargin < 1
        error('areaClassification: dirName is a required input')
    else
        ...
        if(ext.equalsIgnoreCase('gz'))
            ...
            try
                ...
                if isnan(err) && ~isempty(area)
                    ...
                    movefile(char(name.concat('.gz')),char(nPath.concat(char(strcat(char(name.substring(name.lastIndexOf('/')+1)),'.gz')))));
                end
                delete(char(fileT));
                if ~mod(f-2,100)
            ...
            if isequal(dirData(f).isdir,1)
                ...
                areaClassification({newPath,char(savePath.concat(dirData(f).name))});
            end
        end
    end
end
